...
class Cart {
	...
	public function __construct($registry) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE (api_id > '0' AND date_added < DATE_SUB(NOW(), INTERVAL 1 HOUR)) OR (customer_id = '0' AND date_added < DATE_SUB(NOW(), INTERVAL 30 DAY))");
		...
		if (isset($registry)) {
			...
			if ($registry->get('request')) {
				...
				if (isset($request->cookie['cart_session'])) {
					$old_session = $this->db->escape($request->cookie['cart_session']);

					if ($old_session && $old_session != $this->db->escape($this->session->getId())) {
						// Update any guest cart entries from the old session id to the current
						$this->db->query("UPDATE " . DB_PREFIX . "cart SET session_id = '" . $this->db->escape($this->session->getId()) . "' WHERE session_id = '" . $old_session . "' AND customer_id = '0'");
					}
				}

				// Ensure cookie is present for future requests. If missing or differs, set it for 30 days.
				if (!isset($request->cookie['cart_session']) || $request->cookie['cart_session'] != $this->session->getId()) {
					@setcookie('cart_session', $this->session->getId(), time() + 60 * 60 * 24 * 30, '/');
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$product_discount_query = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$cart['product_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND quantity <= '" . (float)$discount_quantity . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY quantity DESC, priority ASC, price ASC LIMIT 1");

				if ($product_discount_query->num_rows && (float)$product_discount_query->row['price'] < (float)$price) {
				...
				if ($product_special_query->num_rows && (float)$product_special_query->row['price'] < (float)$price) {
				...
				$quantity_step = isset($product_query->row['quantity_step']) ? (float)$product_query->row['quantity_step'] : 1.0;
