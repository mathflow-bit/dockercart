...
class ControllerExtensionModuleDockercartSeoGenerator extends Controller {
    ...
    public function index() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('module_dockercart_seo_generator', $this->request->post);
            
            $this->session->data['success'] = $this->language->get('text_success');
            
            $this->response->redirect($this->url->link('extension/module/dockercart_seo_generator', 'user_token=' . $this->session->data['user_token'], true));
        }
        ...
        // Ссылки
        $data['action'] = $this->url->link('extension/module/dockercart_seo_generator', 'user_token=' . $this->session->data['user_token'], true);
        ...
        // Статус модуля
        if (isset($this->request->post['module_dockercart_seo_generator_status'])) {
            $data['module_dockercart_seo_generator_status'] = $this->request->post['module_dockercart_seo_generator_status'];
        } else {
            $data['module_dockercart_seo_generator_status'] = $this->config->get('module_dockercart_seo_generator_status');
        }
        
        // Настройка режима отладки (debug)
        if (isset($this->request->post['module_dockercart_seo_generator_debug'])) {
            $data['module_dockercart_seo_generator_debug'] = $this->request->post['module_dockercart_seo_generator_debug'];
        } else {
            $data['module_dockercart_seo_generator_debug'] = $this->config->get('module_dockercart_seo_generator_debug');
        }
        
        // Размер пакета обработки
        if (isset($this->request->post['module_dockercart_seo_generator_batch_size'])) {
            $data['module_dockercart_seo_generator_batch_size'] = $this->request->post['module_dockercart_seo_generator_batch_size'];
        } else {
            $data['module_dockercart_seo_generator_batch_size'] = $this->config->get('module_dockercart_seo_generator_batch_size') ?: 50;
        }
        
        // Отключение языкового префикса
        if (isset($this->request->post['module_dockercart_seo_generator_disable_language_prefix'])) {
            $data['module_dockercart_seo_generator_disable_language_prefix'] = $this->request->post['module_dockercart_seo_generator_disable_language_prefix'];
        } else {
            $data['module_dockercart_seo_generator_disable_language_prefix'] = $this->config->get('module_dockercart_seo_generator_disable_language_prefix');
        }
        
        // Лицензия
        if (isset($this->request->post['module_dockercart_seo_generator_license_key'])) {
            $data['module_dockercart_seo_generator_license_key'] = $this->request->post['module_dockercart_seo_generator_license_key'];
        } else {
            $data['module_dockercart_seo_generator_license_key'] = $this->config->get('module_dockercart_seo_generator_license_key');
        }
        
        if (isset($this->request->post['module_dockercart_seo_generator_public_key'])) {
            $data['module_dockercart_seo_generator_public_key'] = $this->request->post['module_dockercart_seo_generator_public_key'];
        } else {
            $data['module_dockercart_seo_generator_public_key'] = $this->config->get('module_dockercart_seo_generator_public_key');
        }
        
        // Шаблоны для каждого типа сущности и каждого языка
        // Note: seo_url is now auto-generated from name, not from template
        $entity_types = array('product', 'category', 'manufacturer', 'information');
        $meta_fields = array('meta_title', 'meta_description', 'meta_keyword');
        
        foreach ($entity_types as $entity_type) {
            foreach ($data['languages'] as $language) {
                foreach ($meta_fields as $field) {
                    $key = 'module_dockercart_seo_generator_' . $entity_type . '_' . $field . '_' . $language['language_id'];
                    
                    if (isset($this->request->post[$key])) {
                        $data[$key] = $this->request->post[$key];
                    } else {
                            // Determine language code for this language
                            $lang_code = isset($language['code']) ? $language['code'] : 'en-gb';
                            $data[$key] = $this->config->get($key) ?: $this->getDefaultTemplate($entity_type, $field, $lang_code);
                    }
                }
            }
        }
        ...
        // Статистика
        $data['stats'] = $this->model_extension_module_dockercart_seo_generator->getStatistics();
        ...
        foreach ($entity_types as $etype) {
            ...
            $meta_fields = array('seo_url', 'meta_title', 'meta_description', 'meta_keyword');
            if (isset($entity_data['languages']) && is_array($entity_data['languages'])) {
                foreach ($entity_data['languages'] as $language) {
                    $lang_id = $language['language_id'];
                    $lang_code = isset($language['code']) ? $language['code'] : 'en-gb';

                    foreach ($meta_fields as $field) {
                        $key = 'module_dockercart_seo_generator_' . $etype . '_' . $field . '_' . $lang_id;

                        if (isset($this->request->post[$key])) {
                            $entity_data[$key] = $this->request->post[$key];
                        } else {
                            $entity_data[$key] = $this->config->get($key) ?: $this->getDefaultTemplate($etype, $field, $lang_code);
                        }
                    }
                }
            }
            ...
            $data[$etype . '_entity_html'] = $this->load->view('extension/module/dockercart_seo_generator_entity', $entity_data);
        }

        $this->response->setOutput($this->load->view('extension/module/dockercart_seo_generator', $data));
    }
    ...
    public function preview() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/dockercart_seo_generator')) {
            ...
            $entity_type = isset($this->request->post['entity_type']) ? $this->request->post['entity_type'] : 'product';
            $language_id = isset($this->request->post['language_id']) ? (int)$this->request->post['language_id'] : 1;
            ...
            if (empty($templates)) {
                ...
                foreach ($meta_fields as $field) {
                    $key = 'module_dockercart_seo_generator_' . $entity_type . '_' . $field . '_' . $language_id;
                    $templates[$field] = $this->config->get($key) ?: $this->getDefaultTemplate($entity_type, $field, $lang_code);
                }
            }

            $json['previews'] = $this->model_extension_module_dockercart_seo_generator->generatePreview($entity_type, $language_id, $templates, 10);
            $json['success'] = true;
        }
    ...
    public function generate() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/dockercart_seo_generator')) {
            ...
            $json['error'] = $this->language->get('error_license_required') ?: 'License key is required to use generation feature. Please enter and verify your license key in General Settings.';
        } else {
            // Handle JSON POST data (sent by fetch API with Content-Type: application/json)
            $input_json = file_get_contents('php://input');
            if (!empty($input_json)) {
            ...
            $generate_type = isset($this->request->post['generate_type']) ? $this->request->post['generate_type'] : 'all'; // url, meta, all
            ...
            if (isset($this->request->post['overwrite_url'])) {
                ...
                $this->logger->info('AJAX generate(): overwrite_url flag received = ' . ($overwrite_url ? '1' : '0'));
            } else {
                $filter_empty_url = isset($this->request->post['filter_empty_url']) ? (bool)$this->request->post['filter_empty_url'] : false;
            }
            ...
            if (isset($this->request->post['overwrite_meta'])) {
                ...
                $this->logger->info('AJAX generate(): overwrite_meta flag received = ' . ($overwrite_meta ? '1' : '0'));
            } else {
                $filter_empty_meta = isset($this->request->post['filter_empty_meta']) ? (bool)$this->request->post['filter_empty_meta'] : false;
            }

            // If user requested only URL generation, don't apply meta filters — they can cause 0 results
            if ($generate_type === 'url') {
                $this->logger->info('AJAX generate(): generate_type=url -> forcing filter_empty_meta = false');
                $filter_empty_meta = false;
            }

            // If user requested only META generation, don't apply URL filters
            if ($generate_type === 'meta') {
                $this->logger->info('AJAX generate(): generate_type=meta -> forcing filter_empty_url = false');
                $filter_empty_url = false;
            }
            $offset = isset($this->request->post['offset']) ? (int)$this->request->post['offset'] : 0;
            $batch_size = $this->config->get('module_dockercart_seo_generator_batch_size') ?: 50;
            $templates = isset($this->request->post['templates']) ? $this->request->post['templates'] : array();
                ...
                if ($language_id > 0) {
                    ...
                    foreach ($meta_fields as $field) {
                        ...
                        foreach ($languages as $language) {
                            $lang_id = $language['language_id'];
                            $lang_code = isset($language['code']) ? $language['code'] : 'en-gb';
                            $key = 'module_dockercart_seo_generator_' . $entity_type . '_' . $field . '_' . $lang_id;
                            $templates[$field][$lang_id] = $this->config->get($key) ?: $this->getDefaultTemplate($entity_type, $field, $lang_code);
                        }
                    }
                }
            }
    ...
    private function registerEvents() {
        ...
        // События для товаров
        $this->model_setting_event->addEvent('dockercart_seo_product_add_after', 'admin/model/catalog/product/addProduct/after', 'extension/module/dockercart_seo_generator/eventProductAddAfter');
        $this->model_setting_event->addEvent('dockercart_seo_product_edit_after', 'admin/model/catalog/product/editProduct/after', 'extension/module/dockercart_seo_generator/eventProductEditAfter');
        
        // События для категорий
        $this->model_setting_event->addEvent('dockercart_seo_category_add_after', 'admin/model/catalog/category/addCategory/after', 'extension/module/dockercart_seo_generator/eventCategoryAddAfter');
        $this->model_setting_event->addEvent('dockercart_seo_category_edit_after', 'admin/model/catalog/category/editCategory/after', 'extension/module/dockercart_seo_generator/eventCategoryEditAfter');
        
        // События для производителей
        $this->model_setting_event->addEvent('dockercart_seo_manufacturer_add_after', 'admin/model/catalog/manufacturer/addManufacturer/after', 'extension/module/dockercart_seo_generator/eventManufacturerAddAfter');
        $this->model_setting_event->addEvent('dockercart_seo_manufacturer_edit_after', 'admin/model/catalog/manufacturer/editManufacturer/after', 'extension/module/dockercart_seo_generator/eventManufacturerEditAfter');
        
        // События для информационных страниц
        $this->model_setting_event->addEvent('dockercart_seo_information_add_after', 'admin/model/catalog/information/addInformation/after', 'extension/module/dockercart_seo_generator/eventInformationAddAfter');
        $this->model_setting_event->addEvent('dockercart_seo_information_edit_after', 'admin/model/catalog/information/editInformation/after', 'extension/module/dockercart_seo_generator/eventInformationEditAfter');
    }
     ...
     */
    private function autoGenerateForEntity($entity_type, $entity_id, $force_update = false) {
        // Логирование события для отладки
        $this->logger->info("Event triggered for $entity_type ID: $entity_id, force_update: $force_update");
    ...
    public function eventProductAddAfter($route, $args) {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->session->data['success'])) {
            ...
            if ($product_id) {
                $this->logger->info("Auto-generating for new product: $product_id");
    ...
    public function eventCategoryAddAfter($route, $args) {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->session->data['success'])) {
            $query = $this->db->query("SELECT MAX(category_id) as max_id FROM " . DB_PREFIX . "category");
            $category_id = $query->row['max_id'];
            
            if ($category_id) {
                $this->logger->info("Auto-generating for new category: $category_id");
    ...
    public function eventManufacturerAddAfter($route, $args) {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->session->data['success'])) {
            $query = $this->db->query("SELECT MAX(manufacturer_id) as max_id FROM " . DB_PREFIX . "manufacturer");
            $manufacturer_id = $query->row['max_id'];
            
            if ($manufacturer_id) {
                $this->logger->info("Auto-generating for new manufacturer: $manufacturer_id");
                $this->autoGenerateForEntity('manufacturer', $manufacturer_id, false);
            }
        }
    }
    ...
    public function eventManufacturerEditAfter($route, $args) {
        ...
        if ($manufacturer_id > 0 && isset($this->session->data['success'])) {
            $this->logger->info("Auto-generating for manufacturer: $manufacturer_id");
            $this->autoGenerateForEntity('manufacturer', $manufacturer_id, false);
        }
    }
    ...
    public function eventInformationAddAfter($route, $args) {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->session->data['success'])) {
            $query = $this->db->query("SELECT MAX(information_id) as max_id FROM " . DB_PREFIX . "information");
            $information_id = $query->row['max_id'];
            
            if ($information_id) {
                $this->logger->info("Auto-generating for new information: $information_id");
    ...
    public function eventInformationEditAfter($route, $args) {
        ...
        if ($information_id > 0 && isset($this->session->data['success'])) {
            $this->logger->info("Auto-generating for information: $information_id");
            $this->autoGenerateForEntity('information', $information_id, false);
        }
    }
    ...
    public function uninstall() {
        ...
        // Удаляем события
        $this->db->query("DELETE FROM " . DB_PREFIX . "event WHERE code LIKE 'dockercart_seo_%'");
    }
    
    /**
     * Получить шаблоны по умолчанию из языковых файлов
     */
    private function getDefaultTemplate($entity_type, $field, $language_code = '') {
        ...
        if (!empty($language_code)) {
            $lang_file = DIR_APPLICATION . 'language/' . $language_code . '/extension/module/dockercart_seo_generator.php';
        ...
        if (empty($template) || $template === $lang_key) {
            ...
            $defaults = array(
                'product' => array(
                    ...
                    'meta_description' => '{name} from {manufacturer}. {description}',
            ...
            return isset($defaults[$entity_type][$field]) ? $defaults[$entity_type][$field] : '';
        }
    ...
    private function checkLicenseForGeneration() {
        $license_key = $this->config->get('module_dockercart_seo_generator_license_key');
        ...
        if (!file_exists(DIR_SYSTEM . 'library/dockercart_license.php')) {
            $this->logger->info('Generation allowed: License library not found (backwards compatibility)');
            return true;
        }
        ...
        if (!class_exists('DockercartLicense')) {
            $this->logger->info('Generation allowed: DockercartLicense class not found');
            return true;
        }

        try {
            $license = new DockercartLicense($this->registry);
            $result = $license->verify($license_key, 'dockercart_seo_generator');

            if (!$result['valid']) {
                $this->logger->info('Generation blocked: License validation failed - ' . (isset($result['error']) ? $result['error'] : 'Unknown error'));
                return false;
            }

            $this->logger->info('Generation allowed: License validated successfully');
            return true;
        } catch (Exception $e) {
            $this->logger->info('Generation blocked: License verification exception - ' . $e->getMessage());
            return false;
        }
    }
    ...
    public function verifyLicenseAjax() {
        ...
        try {
            ...
            if (!empty($public_key)) {
                ...
                $result = $license->verifyWithPublicKey($license_key, $public_key, 'dockercart_seo_generator', true);
            } else {
                $this->logger->info('AJAX: Using saved public key from database');
                $result = $license->verify($license_key, 'dockercart_seo_generator', true);
            }
    ...
    public function generateControllers() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/dockercart_seo_generator')) {
            ...
            $json['error'] = $this->language->get('error_license_required') ?: 'License key is required to use generation feature.';
        } else {
            ...
            $language_id = isset($this->request->post['language_id']) ? (int)$this->request->post['language_id'] : 0;
            $controllers = isset($this->request->post['controllers']) ? $this->request->post['controllers'] : array();
            $overwrite = isset($this->request->post['overwrite']) ? (bool)$this->request->post['overwrite'] : false;
            
            if (empty($controllers)) {
                $json['error'] = 'No controllers selected';
            } else {
                $result = $this->model_extension_module_dockercart_seo_generator->generateControllersSeoUrls($controllers, $language_id, $overwrite);
                
                $json['processed'] = $result['processed'];
                $json['skipped'] = isset($result['skipped']) ? $result['skipped'] : 0;
                $json['total'] = $result['total'];
                $json['success'] = true;
            }
        }
    ...
    public function deleteControllers() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/dockercart_seo_generator')) {
            ...
            $controllers = isset($input['controllers']) ? $input['controllers'] : array();
            ...
            if (empty($controllers)) {
                ...
                foreach ($controllers as $controller) {
                    $route = isset($controller['route']) ? $controller['route'] : '';
                    
                    if (!empty($route)) {
                        foreach ($languages as $language) {
                            $success = $this->model_extension_module_dockercart_seo_generator->deleteControllerSeoUrl($route, $language['language_id']);
                            if ($success) {
                                $deleted_count++;
                            }
                        }
    ...
    public function updateControllerUrl() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/dockercart_seo_generator')) {
            ...
            $seo_url = isset($this->request->post['seo_url']) ? $this->request->post['seo_url'] : '';
            $language_id = isset($this->request->post['language_id']) ? (int)$this->request->post['language_id'] : 0;
            ...
            if (empty($route)) {
                ...
                // Обновляем SEO URL
                $success = $this->model_extension_module_dockercart_seo_generator->updateControllerSeoUrl($route, $seo_url, $language_id);
                
                if ($success) {
                    $json['success'] = true;
                    $json['message'] = 'SEO URL updated successfully';
                } else {
                    $json['error'] = 'This SEO URL is already in use by another controller';
                }
            }
        }
    ...
    public function deleteControllerUrl() {
        ...
        if (!$this->user->hasPermission('modify', 'extension/module/dockercart_seo_generator')) {
            ...
            if (empty($route)) {
                ...
                $success = $this->model_extension_module_dockercart_seo_generator->deleteControllerSeoUrl($route, $language_id);
                if ($success) {
                    $json['success'] = true;
